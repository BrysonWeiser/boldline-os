...
export default async (req) => {
  ...
  // Work-photo gallery: real photos only, hero excluded, capped
  const photos = media.filter((m) => m.category === "photo" && m.url && (!hero || m.path !== hero.path)).slice(0, 6);

  const css = `*{box-sizing:border-box;margin:0;padding:0}img{max-width:100%;display:block}body{font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Helvetica,Arial,sans-serif;background:#fff;color:#111827;line-height:1.55}.wrap{max-width:1140px;margin:0 auto;padding:0 20px}.topbar{display:flex;align-items:center;justify-content:space-between;gap:12px;padding:14px 0}.tb-name{font-weight:800;font-size:15px;color:#0F172A}.tb-call{font-size:14px;font-weight:700;color:#0F172A;text-decoration:none;white-space:nowrap}.hero{padding:26px 0 32px;background:linear-gradient(180deg,#FAF9F5,#fff)}.hero-g{display:grid;gap:28px;align-items:center}.eyebrow{font-size:12px;font-weight:800;letter-spacing:.08em;text-transform:uppercase;color:${GOLD};margin-bottom:10px}.headline{font-size:clamp(28px,5vw,44px);font-weight:800;color:#0F172A;line-height:1.15;margin-bottom:12px}.subhead{font-size:clamp(15px,2vw,18px);color:#4B5563;margin-bottom:20px;max-width:56ch}.cta{display:inline-block;padding:15px 26px;font-size:16px;font-weight:800;border-radius:12px;border:none;background:${GOLD};color:#1A1502;cursor:pointer;text-align:center;text-decoration:none}.ctarow{display:flex;align-items:center;gap:16px;flex-wrap:wrap}.callrow{font-size:14px;color:#6B7280}.callrow a{color:#0F172A;font-weight:800;text-decoration:none}.heroimg{width:100%;aspect-ratio:4/3;object-fit:cover;border-radius:18px;box-shadow:0 18px 40px rgba(15,23,42,.14)}.chips{display:flex;flex-wrap:wrap;gap:10px;padding:18px 0;border-top:1px solid #F1EFE9;border-bottom:1px solid #F1EFE9}.chip{display:flex;align-items:center;gap:8px;font-size:13.5px;font-weight:600;color:#374151;background:#FAF9F5;border:1px solid #EFEBDF;border-radius:999px;padding:8px 14px}.sec{padding:38px 0}.sec.tight{padding-top:0}.sec-t{font-size:clamp(20px,3vw,28px);font-weight:800;color:#0F172A;margin-bottom:18px}.bene{display:grid;gap:12px;grid-template-columns:1fr}.bcard{display:flex;gap:12px;align-items:flex-start;background:#FAF9F5;border:1px solid #EFEBDF;border-radius:14px;padding:16px}.bcard span{font-size:15px;color:#1F2937;font-weight:600}.check{flex-shrink:0;width:24px;height:24px;border-radius:50%;background:rgba(200,168,75,.16);color:${GOLD};display:flex;align-items:center;justify-content:center;font-size:13px;font-weight:800}.gal{display:grid;gap:10px;grid-template-columns:1fr 1fr}.gal img{aspect-ratio:4/3;object-fit:cover;border-radius:12px;width:100%}.offer{background:#0F172A;border-radius:20px;padding:30px 24px;text-align:center;color:#fff}.offer .eyebrow{margin-bottom:8px}.offer-t{font-size:clamp(19px,2.6vw,26px);font-weight:800;margin-bottom:18px}.formsec{background:#F8F7F2;padding:44px 0 54px}.fcard{max-width:560px;margin:0 auto;background:#fff;border:1px solid #ECE9DF;border-radius:18px;padding:26px 22px;box-shadow:0 14px 34px rgba(15,23,42,.07)}.formtitle{font-size:20px;font-weight:800;margin-bottom:4px;color:#0F172A;text-align:center}.formsub{font-size:13.5px;color:#6B7280;text-align:center;margin-bottom:18px}.inp{width:100%;padding:14px;border:1px solid #E5E7EB;border-radius:11px;font-size:15px;margin-bottom:10px;font-family:inherit;background:#fff}.err{display:none;font-size:12.5px;color:#DC2626;margin-bottom:10px;text-align:center}.thanks{display:none;text-align:center;padding:18px 6px}.thanks h2{font-size:19px;margin-bottom:6px;color:#0F172A}.thanks p{font-size:13.5px;color:#6B7280}.foot{padding:22px 0 30px;text-align:center;font-size:13px;color:#6B7280}.foot a{color:#0F172A;font-weight:700;text-decoration:none}@media(min-width:640px){.bene{grid-template-columns:1fr 1fr}.gal{grid-template-columns:1fr 1fr 1fr}}@media(min-width:940px){.hero{padding:44px 0 50px}.hero-g.has-img{grid-template-columns:1.05fr .95fr}.bene{grid-template-columns:repeat(auto-fit,minmax(240px,1fr))}}`;
  ...
  const page = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(lp.headline)} — ${esc(cl.name)}</title><meta name="description" content="${esc(lp.subheadline || "")}"><meta property="og:title" content="${esc(lp.headline)} — ${esc(cl.name)}"><meta property="og:description" content="${esc(lp.subheadline || "")}">${hero ? `<meta property="og:image" content="${esc(hero.url)}">` : ""}<style>${css}</style></head><body>

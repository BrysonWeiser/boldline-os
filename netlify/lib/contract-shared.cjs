...
const resultWords = (cl) => {
  ...
  if (String(c.billingResultKind || "") !== "sale") {
    return { kind:"lead", one:"Qualified Lead", many:"Qualified Leads", per:"per qualified lead",
      countNoun:"Lead counts", itNoun:"lead", countSource:'campaign tracking data', feeName:'Per-Qualified-Lead Fee',
  ...
  return { kind:"sale", one:"Qualified Sale", many:"Qualified Sales", per:"per qualified sale",
    ...
    countSource:'Client&rsquo;s own order records', feeName:'Per-Qualified-Sale Fee',
...
const makeContractHTML=(cl,pkg,LOGO)=>{
  ...
  // ── 🔴 WHICH VERSION OF THE TERMS THIS CLIENT IS ON ───────────────────────────
  // Bryson, 2026-09-02: *"make sure in the contract from now on say something like if a
  // client fails to respond, complete intake form, etc. the contract is voided and there is
  // a fee charged as well (this will be for future clients not Sebastian until he renews
  // his contract)"*.
  //
  // 🔴 A SIGNED AGREEMENT MUST NEVER GAIN A CLAUSE IT WAS NOT SIGNED WITH. This document
  // renders FRESH every time anyone opens it, in the portal and in the OS, so adding a
  // section unconditionally would put new obligations into a contract a client already put
  // their name to, retroactively and silently. That is the worst thing this file can do.
  //
  // So terms are VERSIONED. Version 1 is everything signed before 3 September 2026, which
  // today is exactly one client. Version 2 adds the delay-and-abandonment section below.
  // The version is read from the record when it is set, and otherwise inferred: unsigned
  // means the newest terms, signed-before-the-cutoff means the terms of that day.
  //
  // 🔴 RENEWAL MUST STAMP THE NEW VERSION, and the inference alone cannot do it, because
  ...
  // v5 — the Start Date is the day the ads go live, and the Effective Date splits off from it.
  // 🔴 Declared HERE rather than beside the rest of the v5 block below, because the Client
  ...
  const nAd=nBase+1, nResp=nBase+2, nAband=nBase+3,
        nNoG=nBase+3+dAb, nTerm=nBase+4+dAb, nIP=nBase+5+dAb, nConf=nBase+6+dAb,
        nData=nBase+7+dAb, nLiab=nBase+8+dAb, nInd=nBase+9+dAb, nRel=nBase+10+dAb,
        nLaw=nBase+11+dAb, nGen=nBase+12+dAb;

  // 🔴 The fee is framed as amounts already earned plus reserved capacity, NOT as a penalty,
  // and that framing is the difference between a clause that holds and one a court strikes
  // out. A flat punishment for walking away is unenforceable in most US states unless it is
  // a genuine estimate of loss. The setup fee pays for work already done, and the monthly
  // minimum reflects a slot held open while other work was turned away. Both are real.
  const abandonHTML = !hasAbandon ? "" : (
    '<h2>'+nAband+'. Client Delay and Abandonment</h2>'
   +'<p>(a) Agency cannot begin or continue work without the intake information, account access, and approvals described above. Client will provide these within fourteen (14) days of the Effective Date, and will respond to Agency reasonably promptly after that.</p>'
  ...
  // 🔴 v5 — THE START DATE IS AN EVENT, NOT A GUESS.
  //
  // Bryson, 2026-09-17: *"yea do that"*, on stopping a guessed date from costing an amendment
  // every time a launch slips. Until v5 the Key Terms named a date somebody picked before
  // anyone knew when the client would hand over their ad account, and that date is an operative
  // term of a signed, frozen PDF. Moving it needed a signed amendment, which this agreement
  // itself requires (the email carve-out covers only upgrades and renewals). It happened to
  // Stencil & Thread, who were given the concession by hand, and was about to happen to the
  // next client. Twice is a pattern, so it became the wording.
  //
  // From v5 the Start Date IS the day the first campaign begins delivering. A slip is the
  // agreement working as written rather than a change to it, so there is nothing to amend and
  // nothing to re-sign. `ads-sync` recognises that day from the platforms' own spend reporting
  // and records it (see netlify/lib/campaign-live.mjs).
  //
  // 🔴 AND THE EFFECTIVE DATE SPLITS OFF FROM IT. Before v5 both were the same field, which had
  // the intake clock running from a date AFTER the day we need the intake in order to launch at
  // all. From v5 the agreement takes effect on signature, where the fourteen-day handover clock
  // belongs, and the TERM starts when the ads do, where the fees belong.
  const liveOn = eventStart && cl.campaignLiveAt
    ? new Date(cl.campaignLiveAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })
    : "";
  const startCell = !eventStart
    ? (esc(cl.contractStart) || "\u2014")
    : liveOn
      ? esc(liveOn)
      : (cl.contractStart
        ? "The day the ads go live<br><span style=\"font-size:9px;color:#666\">estimated " + esc(cl.contractStart) + "</span>"
        : "The day the ads go live");
  const endCell = !eventStart
    ? (esc(cl.contractEnd) || "\u2014")
    : (liveOn || !cl.contractEnd)
      ? (esc(cl.contractEnd) || "\u2014")
      : monthsLabel(termMo) + " after the Start Date<br><span style=\"font-size:9px;color:#666\">estimated " + esc(cl.contractEnd) + "</span>";
  ...
  const meta =
   ...
   +metaItem("Start Date",startCell)
   +metaItem("End Date",endCell)
  ...
  const perfSection = hasPerf ? (
   ...
   +(perLeadFee ? (
      '<p>4.2 <strong>Performance Fee ('+W.per+').</strong> The Performance Fee for a month is the '+W.feeName+' stated above multiplied by the number of '+W.many+' generated by the Campaigns in that month. '+W.defn+'</p>'
    ) : (
  ...
  const css='body{font-family:Georgia,"Times New Roman",serif;font-size:12.5px;line-height:1.75;color:#191919;background:#fff;padding:34px 38px;max-width:100%;word-break:break-word}'
   ...
   // 🔴 The wordmark needs 204px on one line. Inside the portal's iframe the contract is
   // only 274px wide on a 360px phone and 304px on a 390px one, so BOLDLINE and MEDIA
   // split across two lines beside the logo and read as a squeeze rather than a mark.
   // Below 460px it becomes a centred stack, which is what a letterhead does anyway.
   // Print is unaffected: a sheet of paper is ~816px, so it keeps the row.
   +'@media(max-width:460px){.hd{flex-direction:column;text-align:center;gap:8px;padding-bottom:12px}.hd img{width:46px;height:46px}}'
   // And a second step for the narrowest phones: at a 360px screen the contract is only
   // 274px wide, which still leaves the 21px wordmark ~22px short of fitting on one line.
   +'@media(max-width:380px){.hd .co{font-size:18px}.hd .tag{font-size:9px;letter-spacing:.1em}}'
   // 🔴 THESE BREAKPOINTS ARE THE CONTRACT'S OWN WIDTH, NOT THE PHONE'S. It renders in an
   // iframe, so the media query sees the frame: a 390px phone gives the contract 304px and
   // a 360px phone gives it 274px. A 340px cutoff therefore fired on ordinary phones and
   // shrank the wordmark to 14px when 18px fitted fine. 260px catches only a 320px screen,
   // which is where 18px genuinely will not fit.
   +'@media(max-width:260px){.hd .co{font-size:14px;letter-spacing:.02em}}'
  ...
  return '<!DOCTYPE html><html><head><meta charset="UTF-8"><style>'+css+'</style></head><body>'
   ...
   +(oneTime
     ...
     : (introOnly
        ...
        : (hasFullTermExit
           // 🔴 v3. Bryson, 2026-09-02: cancelling early should cost the REST of the term,
           // not one month. He committed capacity for the whole term and turned other work
           // away, so the remaining minimums are the loss. Kept in the liquidated-damages
           // frame the old clause already used, because that frame is the only reason an
           // acceleration like this survives being challenged.
           //
           // 🔴 A PART MONTH COUNTS AS A WHOLE ONE, at THEIR OWN rate. The alternative he
           // floated was a flat $400 for a final part month, and a flat number bearing no
           // relation to their contract is precisely the shape a court reads as a penalty
           // rather than as a genuine estimate of loss. Worse, on every package above $400
           // it would make cancelling in the last month CHEAPER than seeing it out, which
           // rewards exactly the behaviour the clause exists to discourage.
           ? '<p>(b) <strong>By Client, early.</strong> Client may terminate before the End Date by written notice to Agency at the email address on page one, effective when Agency receives it. Client pays, as liquidated damages and not as a penalty: (i) all fees accrued through the effective termination date, plus (ii) the <strong>Monthly Minimum for each remaining month of the Committed Term</strong>, where any part of a month counts as a whole month, plus (iii) if Client received a discounted rate in exchange for the Committed Term, the difference between the Standard Rate ('+money(stdMonthly)+'/mo) and the discounted rate actually paid for each month already billed in the current term. The Setup Fee is earned on signing and is not refundable in any amount. The parties agree these amounts are a reasonable estimate of Agency&rsquo;s losses, which are difficult to calculate precisely: Agency reserves capacity for the whole Committed Term and turns away other work to hold it.</p>'

// Shared by ad-generator (sync) and ad-generator-background (long jobs).
//
// Split out because the Google action blew the ~10s synchronous Netlify limit on
// its first real use and returned a 504: 3-5 ad groups x 15 headlines is far more
// output than a sync function can finish. The prompts, tools, limits and output
...
export const HONESTY = `WHAT YOU MAY NOT CLAIM:
...
export async function runTool({ system, prompt, tool, maxTokens }) {
...
// here so every existing caller and test is untouched. The reason for the move: the
// public landing page renderer needs the same word-safe trimming, and importing it from
// this file would drag the Anthropic SDK into a page render. One definition, no copy.
export { fitWords, fitPhrase } from "./humanize.mjs";
...
export const brief = (b) => {
  const L = [];
  const push = (label, v) => { if (v && String(v).trim()) L.push(`${label}: ${String(v).trim().slice(0, 300)}`); };
  push("Business", b.name);
  // 🔴 THE OFFER IS A BRIEF, NOT A LINE TO REUSE. Bryson, 2026-08-24: "I dont want the ai
  // to take it word for word I just want it to use what I put in as the idea of what it
  // should do then to rewrite it itself so it sounds good." Labelled so the model cannot
  // read it as approved copy, which is exactly how a typed note ends up in a headline.
  push("What they sell (this is a note from the owner, not ad copy: use the MEANING and write your own words)", b.offer);
...
// ── Meta + creative cleaning, shared so both paths behave identically ────────
export const cleanMeta = (data) =>
...
export const systemFor = (isAgency) => {
  ...
  return `You are a paid search specialist who builds campaigns that have to make money on a small budget. You are not a copywriter writing brand slogans.
...
export const promptFor = (action, B) => {
  if (action === "google") return `Build a Google Search campaign.
...
Then write the operator note: what this structure bets on, and what to watch in week one and two.`;
  ...
  if (action === "meta") return `Write Meta (Facebook and Instagram) ad copy.
...
export const friendlyError = (m) =>
  ...
  : /timeout|timed out|ETIMEDOUT/i.test(m) ? "That took too long to generate. Try again."

...
**GOTCHA — `pageSize` is rejected, not ignored.** The first live use of the detail read failed with `Setting the page size is not supported. Search Responses will have fixed page size of '10000' rows. [requestError=PAGE_SIZE_NOT_SUPPORTED]`. Google Ads does not accept `pageSize` on `googleAds:search` at all; the response is fixed at 10000 rows, which is far more than one campaign's ad groups, ads or keywords will ever be. Removed, and the tests now assert no detail query sends it. Note the error surfaced cleanly on the first try because of the improved `apiErrMsg` — the request-level code named the problem outright.

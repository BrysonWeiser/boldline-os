...
- **Enable Realtime** (for instant OS updates on `clients` and `website_leads`): Supabase dashboard → **Database → Replication** → the `supabase_realtime` publication → toggle the table on. Without it, the OS still updates within ~15-20s via polling fallback (and instantly on tab-focus).

...
**🔴 CORRECTION 2026-08-14 (same day) — the derivation read an adPerf shape that does not exist.** As first shipped, `deriveBotStatuses` did `Array.isArray(perf.campaigns)` and `perf.spend30`. **`ads-sync` writes neither.** Its snapshot is COUNTS: `adPerf.google.campaigns`/`.live` (numbers), the same under `.meta`, a rolled-up `adPerf.totals.{liveCampaigns, spend30d, conversions}`, and `adPerf.budget.{monthly, pacing, pct, state}`. Against real data `camps` was always `[]`, so **keywords, ads, qc, tracking-done, search terms, performance and budget-active would have sat at their lowest state forever, with nothing erroring.** Fixed in both mirrored copies; see KB `house-health-score` for the full note. **The lesson worth keeping:** the 71 passing tests used fixtures invented from the field names I expected instead of read out of `ads-sync.mjs`. When asserting against another job's output, build the fixture from that job's source.

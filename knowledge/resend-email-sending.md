...
- **KEY PERMISSION GOTCHA (2026-08-05):** Resend keys are **"Full access"** or **"Sending access."** A sending-access key can `POST /emails` but **401s** (`"restricted to only send emails"`) on `/contacts`, `/broadcasts`, `/segments`. So any function that manages contacts/segments or sends a **broadcast** (the newsletter: `subscribe.mjs`, `newsletter-shared.sendBroadcast`, `getSubscriberStats`) needs a **Full-access** key. Plain transactional `sendEmail` (reports, client emails) works with either. When contacts silently fail to appear in Resend, check the Netlify **function log** for the 401 — the calling function fail-soft-returns 200 so the UI hides it. Both sites now use full-access keys.
- **Domain verified (2026-08-05):** boldlinemedia.com is verified in Resend, so `REPORTS_FROM_EMAIL` on the OS site is now `BoldLine Media <hello@boldlinemedia.com>` — real branded, deliverable to anyone (previously `onboarding@resend.dev`, which only reaches the account owner). See `domain-dns-wix`, `newsletter-emails`.

...
status: verified
summary: The OS compiles its own ~1.1 MB of JSX in the browser on EVERY page load. The `env` preset was doing that AND rewriting all modern JavaScript down to 2015 syntax, for browsers nobody uses. Removing one word took time-to-first-render from 10.5 s to 3.5 s in a real browser. The remaining 3.5 s is Babel running at all; killing that needs a build step, which also drops a 2.8 MB download. Not yet done.
verified: 2026-08-27
---
...
is his pipeline and his call. Offered 2026-08-27, not yet taken up.

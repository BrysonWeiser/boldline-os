...
**Twilio caveat:** SMS uses the same Twilio path as the report digests; on the trial account A2P 10DLC blocks the texts (fails soft, logged). Email + in-app work now; SMS completes when Twilio is upgraded to paid (see the deploy-status note in `ARIADeployTab`).

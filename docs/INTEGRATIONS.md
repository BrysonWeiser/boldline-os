...
- **[google-ads-api](../knowledge/google-ads-api.md)** &mdash; &#9989; verified &middot; 2026-07-19  
  Google Ads is wired via one MCC + Developer Token + OAuth refresh token (login-customer-id header); code in google-ads.mjs (test/campaigns/setBudget/setStatus). Developer Token BASIC ACCESS APPROVED 2026-07-19 (15,000 ops/day) — the long-standing blocker is cleared. Use the Web-app OAuth client (Desktop got redirect_uri_mismatch). Remaining: confirm the Deploy-tab test card authenticates end-to-end, then wire approve→execute.  
